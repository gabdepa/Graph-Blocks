...
void DFS(Graph *graph, int u, bool *visited, int *disc, int *low, int *parent, bool *articulation_point, int *current_vertices, int *current_edges)
{
    ...
    for (AdjListNode *v = graph->array[u].head; v != NULL; v = v->next)
    {
        ...
        if (!visited[v_id])
        {
            ...
            if ((parent[u] == -1 && (*current_edges > 1)) || (parent[u] != -1 && low[v_id] >= disc[u]))
    ...
    *num_blocos = 0;
    ...
    for (int i = 0; i < V; i++)
    {
        if (!visited[i])
        {
            ...
            {
                blocos[*num_blocos].vertices = current_vertices;
                blocos[*num_blocos].arestas = current_edges; // Contar corretamente as arestas
                (*num_blocos)++;
            }
        }
    }
